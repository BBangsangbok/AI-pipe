Human vs. AI Story Classification Project
...
Why Naïve?: It relies purely on surface-level formatting cues and lacks any semantic understanding of the content.
...
Environment: Trained on a single NVIDIA RTX 3050 Laptop GPU.
...
The Naïve Baseline achieved a surprisingly high accuracy of 92.49%. This indicates that the dataset relies heavily on formatting artifacts (e.g., AI formatting its output with Markdown).
...
Baseline Prediction: Human (0) (Wrong)
...
Baseline Prediction: AI (1) (Wrong)
...
Dataset Simplicity: The fact that a simple keyword baseline achieved ~92.5% suggests the dataset is "too easy" and heavily dependent on formatting artifacts rather than deep linguistic differences.

Watermarks: As seen in Case 2 (API Error), the model likely learned specific "watermarks" (e.g., "OpenAI", system errors) rather than the subtleties of human vs. AI writing styles.

Generalization Concern: The model is likely overfitted to the specific quirks of this dataset. It may perform poorly on "clean" AI text generated by newer models (e.g., GPT-4o) that do not leave such obvious artifacts.
...
Run pipeline_demo.ipynb using Jupyter Notebook.

The notebook handles data loading, preprocessing, training, and evaluation end-to-end.